...
class GraphMetrics:
    ...
    def simple_metrics(graph: DiGraph) -> Dict[str, Any]:
        ...
        metrics = {'nodes': graph.number_of_nodes(), 'edges': graph.number_of_edges(),
    ...
    def connectivity_metrics(graph: DiGraph) -> Dict[str, Any]:
        ...
        metrics = {'parents': f'{np.mean(num_parents):.2f}$\pm${np.std(num_parents):.2f}',
                   'max_parents': np.max(num_parents),
                   'children': f'{np.mean(num_children):.2f}$\pm${np.std(num_children):.2f}',
