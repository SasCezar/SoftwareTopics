...
class LLMCompletion(AbstractCompletion):
    ...
    def complete(self, taxonomy: Taxonomy) -> Taxonomy:
        ...
        for term in tqdm(taxonomy.terms):
            ...
            if len(res) == 1 and res[0] == "None":
                continue
            taxonomy.missing.pop(taxonomy.missing.index(term))
            for hypernym in res:
        ...
        taxonomy.terms = list(set(taxonomy.terms) | set(flatten(taxonomy.pairs)).difference(taxonomy.missing))
